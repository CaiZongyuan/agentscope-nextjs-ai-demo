...
class ResponsesAdapter:
        ...
        if agent_message_list:
            for message in agent_message_list:
                ...
                if message.type == MessageType.MCP_TOOL_CALL:
                    tool_call_message = self._convert_mcp_tool_call_to_output_message(
                        message,
                    )
                    messages.append(tool_call_message)
                if message.type == MessageType.REASONING:
                    reasoning_message = self._convert_reasoning_to_output_message(
                        message,
                    )
            ...
            # Handle Input item list (array) type
                ...
                for input_item in input_data:
                    ...
                    if item_type == "message":
                        ...
                        message = (
                            self._convert_responses_input_message_to_agent_message(  # noqa: E501
                                input_item,
                            )
                        )
                        input_messages.append(message)
                        ...
                        message = self._convert_custom_tool_call_output_to_message(
                            input_item,
                        )
                        input_messages.append(message)
                        ...
                        message = self._convert_function_call_output_to_message(
                            input_item,
                        )
                        input_messages.append(message)
    ...
    def _convert_content_item_to_agent_content(self, content_item):
        ...
        elif content_type == "input_file" and (file_url or file_id or file_data):
            return FileContent(
    ...
    def _get_add_output_index(self, message_id: str):
        ...
        if (
            ...
            self._message_content_index_map[message_id]["output_index"] = output_index
    ...
    def _handle_new_message(
        ...
        if message_event.type == MessageType.FUNCTION_CALL:
            ...
            # Add reasoning_summary_part.added event for Vercel AI SDK
            ...
            messages.append(summary_part_added_event)
    ...
    def _convert_message_content_to_responses_event(
        ...
        if content_type == ContentType.REFUSAL:
            ...
            if content_status == "in_progress":
                content_in_progress_event = self._create_refusal_text_delta_event(
                    content_event,
                    output_index,
                )
                events.append(content_in_progress_event)
    ...
    def _convert_function_call_content_to_responses_event(
        ...
        if content_type == ContentType.DATA:
            if new_content:
                add_event = self._create_function_call_arguments_add_output_item_event(
                    content_event=content_event,
                    output_index=output_index,
                )
            ...
            if isinstance(function, dict):
                ...
                if content_status == "in_progress":
                    delta_event = self._create_function_call_arguments_delta_event(
                        content_event,
                        arguments,
                        output_index,
                    )
                ...
                if content_status == "completed":
                    done_event = self._create_function_call_arguments_done_event(
                        content_event,
                        arguments,
                        output_index,
                    )
    ...
    def _create_reasoning_text_delta_event(
        ...
        """
        # sequence_number will be set uniformly in responses_service
        # Use new event type for Vercel AI SDK compatibility
    ...
    def _convert_agent_error_to_responses_error(
        ...
        try:
            ...
            mapped_code = error_code if error_code in valid_codes else "server_error"
